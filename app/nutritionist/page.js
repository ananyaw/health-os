...
import { getDayPlanForDate, getDishSpec } from "../chef/page";
...
//
// Renamed from "Nutrition" to "Nutritionist" — the persona that helps
// you track what you eat, distinct from Chef who plans and cooks it.
// The "From your Chef" section below reads Chef's weekly plan directly
// (same module, imported) so the two agents share one source of truth
// for what's in rotation — though it currently only reflects Chef's
// default weekly template, not any live edits you've made in Chef this
// session, since that state isn't persisted to Supabase yet.
...
export default function Nutritionist() {
  ...
  const [mealprepOn, setMealprepOn] = useState(null); // null = unknown yet, from onboarding's module toggle
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      ...
      if (data && data.length > 0 && data[0].answers) {
        setMealprepOn(!!(data[0].answers.modules && data[0].answers.modules.mealprep));
      } else {
        setMealprepOn(false);
      }
    }
  ...
  const chefDishes = mealprepOn ? getDayPlanForDate(selectedDate, todayIso, {}).dishes : [];
  ...
  function openLogFromChef(dish) {
    if (isFuture) return;
    const spec = getDishSpec(dish.name);
    setModalState({
      iso: selectedDate,
      slot: MEAL_SLOTS.includes(spec.forMeal) ? spec.forMeal : guessSlotByTime(),
      item: { name: dish.name, calories: spec.calories, protein: spec.protein },
    });
  }
  ...
  return (
    <main style={{ padding: 24, maxWidth: 480, margin: "0 auto", fontFamily: "sans-serif" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>Nutritionist</h1>
      ...
      {mealprepOn && chefDishes.length > 0 && (
        <div style={cardStyle("#ea580c")}>
          <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 8 }}>🧑‍🍳 From your Chef</div>
          {chefDishes.map((d, i) => {
            const spec = getDishSpec(d.name);
            return (
              <div key={d.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderTop: i > 0 ? "1px solid #eee" : "none" }}>
                <div>
                  <div style={{ fontSize: 13, color: "#333" }}>{d.name}</div>
                  <div style={{ fontSize: 11, color: "#999" }}>
                    {spec.forMeal} · {spec.calories} kcal · {spec.protein}g protein
                  </div>
                </div>
                {!isFuture && (
                  <div onClick={() => openLogFromChef(d)} style={{ fontSize: 12, color: "#2563eb", cursor: "pointer", flexShrink: 0, marginLeft: 10 }}>
                    Log this
                  </div>
                )}
              </div>
      ...
      <p style={{ fontSize: 12, color: "#999", marginTop: 8 }}>
        Logging is real (saved to Supabase). "From your Chef" reflects Chef's default weekly plan, not live edits made
        in Chef this session — full cross-agent sync needs Chef's plan persisted to Supabase too, a natural next step.
        AI estimate is still mocked.
      </p>
